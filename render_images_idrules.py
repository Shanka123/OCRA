...
parser.add_argument('--output_image_dir', default='output/idrules_images/train_ood/',
...
def main(args):
  prob_answer_arr = np.load("identity_rules_ood_train.npz")
  ...
  for num_ex in range(10000):
    ...
    for i in range(4):
      ...
      all_feats = np.concatenate([prob_arr[num_ex][:5],np.expand_dims(prob_arr[num_ex][5+i],axis=0)],axis=0)
